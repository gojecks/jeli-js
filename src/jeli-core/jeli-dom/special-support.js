...
events.special.remove = {
  ...
  if(window.jQuery)
  {
      jQuery.support = $isSupport;
      jQuery.fn.injector = domElementProvider.injector;
      jQuery.fn.jModel = domElementProvider.jModel;
      jQuery.fn.compile = domElementProvider.compile;
      jQuery.fn.getFirstChild = domElementProvider.getFirstChild;
      jQuery.cookie = $cookie;

      function jQueryExtraParser(fname){

        return jQuery.fn[fname];
      }
      //extend jquery functiona
      for(var fns in jQuery.fn){
        if(!domElementProvider[fns]){
          domElementProvider[fns] = jQueryExtraParser(fns);
        }
      }
