...
import { isstring, inarray } from 'js.helpers/helpers';
 ...
 * @param {*} injector 
 */
export function WebStateProvider(injector) {
    var $stateCollection = {},
        $intentCollection = {},
        matchers = ["component"],
        _unregistered = {},
    ...
    function generateRoute(routeConfig, requireParent) {
        ...
        if (routeConfig.resolver) {
            for (var prop in routeConfig.resolver) {
                if (isstring(routeConfig.resolver[prop])) {
                    routeConfig.resolver[prop] = injector.get(routeConfig.resolver[prop]);
                }
            }
        }
